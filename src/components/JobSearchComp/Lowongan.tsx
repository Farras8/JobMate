...
const Lowongan: React.FC<LowonganProps> = ({ jobs, isLoading, error }) => {
  ...
  if (isLoading) return <div className="text-center py-8 lg:py-16 text-gray-700 px-4"><div className="inline-flex items-center space-x-2"><div className="w-5 h-5 lg:w-6 lg:h-6 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"></div><span className="text-base lg:text-lg font-medium">Memuat lowongan...</span></div></div>;
  if (error) return <div className="text-center py-8 lg:py-16 text-red-600 bg-red-50 rounded-xl lg:rounded-2xl border border-red-200 mx-2 lg:mx-4"><div className="text-base lg:text-lg font-semibold px-4">{error}</div></div>;
  if (!jobs || jobs.length === 0) return <div className="text-center py-8 lg:py-16 text-gray-600 bg-gray-50 rounded-xl lg:rounded-2xl border border-gray-200 mx-2 lg:mx-4"><div className="text-base lg:text-lg font-medium px-4">Tidak ada lowongan yang ditemukan.</div></div>;
