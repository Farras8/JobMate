...
const JobSearch: React.FC = () => {
  ...
  const fetchAndFilterJobs = async (apiFilters: SearchJobFilters, page: number = 1) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  const debouncedSearch = useCallback(
    debounce((filters: SearchJobFilters, page: number) => {
      fetchAndFilterJobs(filters, page);
    }, 500),
    []
  );
  ...
  const applyUrlFilters = () => {
    ...
    if (companyParam) setKeyword(companyParam);
    if (jobTitleParam) setMainSearchTerm(jobTitleParam);
    if (cityParam) setLocation(cityParam);
    if (jobTypeParam && jobTypeParam !== "Semua Tipe") setSelectedJobType(jobTypeParam);
    ...
    if (jobTypeParam && jobTypeParam !== "Semua Tipe") apiFilters.jobType = jobTypeParam;

    fetchAndFilterJobs(apiFilters, currentPage);
  };
  ...
  useEffect(() => {
    ...
    if (hasUrlParams) {
      ...
      fetchAndFilterJobs({}, currentPage);
    }
        ...
        <div className="max-w-[1700px] mx-auto">
          <div className="text-center mb-12">
            ...
            <h1 className="text-4xl md:text-6xl font-bold bg-gradient-to-r from-gray-900 via-blue-900 to-purple-900 bg-clip-text text-transparent leading-tight mb-6">
              ...
              Pekerjaan Inklusif<br />Pilihanmu!
            </h1>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900">
                      {isLoading ? "Mencari lowongan..." : (totalJobs > 0 ? `${totalJobs} Lowongan Ditemukan` : "Lowongan Untukmu")}
                    </h2>
                ...
                {!isLoading && totalJobs > 0 && (
                  <div className="hidden sm:flex items-center space-x-2 text-sm text-gray-500">
                    <span>Halaman {currentPage} dari {Math.ceil(totalJobs / JOBS_PER_PAGE)}</span>
                  </div>
                )}
              </div>
            </div>
