...
const OverallScoreCard: React.FC<OverallScoreCardProps> = ({ 
    ...
    const getPerformanceText = (score: number) => {
        if (score >= 85) return { title: 'Excellent Performance', desc: 'Resume Anda sangat berkualitas dan siap untuk melamar pekerjaan impian.' };
        if (score >= 70) return { title: 'Good Performance', desc: 'Resume Anda di atas rata-rata namun masih ada ruang untuk peningkatan.' };
        if (score >= 60) return { title: 'Average Performance', desc: 'Resume Anda memerlukan beberapa perbaikan untuk meningkatkan daya saing.' };
            ...
            <div className="relative">
                ...
                <div className="flex flex-col lg:flex-row items-center gap-8">
                    <div className="flex-shrink-0">
                        <div className="relative w-36 h-36">
                            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                                <circle cx="50" cy="50" r="45" stroke="#f3f4f6" strokeWidth="8" fill="none" className="drop-shadow-sm" />
                                <circle cx="50" cy="50" r="45" stroke="url(#scoreGradient)" strokeWidth="8" strokeDasharray={strokeDasharray} strokeLinecap="round" fill="none" className="transition-all duration-1000 ease-out drop-shadow-sm" />
                                <defs>
                    ...
                    <div className="flex-1 text-center lg:text-left">
                        <div className="mb-4">
                            <h4 className={`text-lg font-bold ${getScoreColor(overallScore)} mb-2`}>{performance.title}</h4>
                            <p className="text-sm text-gray-600 leading-relaxed">{performance.desc}</p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {scoreData.map((item, index) => {
                                ...
                                return (
                                    <div key={index} className={`group bg-gradient-to-br ${item.bgColor} border border-gray-200/50 rounded-2xl p-4 transition-all duration-300 transform hover:scale-105 hover:shadow-lg relative overflow-hidden`}>
                                        ...
                                        <div className="relative">
                                            ...
                                            <div className={`text-xl font-bold ${getScoreColor(item.value)} mb-1`}>{item.value}</div>
