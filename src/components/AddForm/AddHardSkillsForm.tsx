...
const AddHardSkillsForm: React.FC<AddHardSkillsFormProps> = ({ onClose, onAddSuccess, existingSkillNames }) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (skillsToSubmit.length === 0) {
      Swal.fire('Input Kosong', 'Mohon isi setidaknya satu hard skill dengan nama dan level.', 'warning');
      return;
    }
    ...
    if (duplicateSkillNames.length > 0 && newSkillsPayload.length > 0) {
        const result = await Swal.fire({
            ...
            html: `Hard skill berikut sudah ada dan tidak akan ditambahkan lagi: <br/><strong>${duplicateSkillNames.join(', ')}</strong>.<br/>Lanjutkan menambahkan skill lainnya?`,
            icon: 'warning',
        ...
        if (!result.isConfirmed) {
         ...
         Swal.fire('Semua Skill Duplikat', `Hard skill berikut sudah ada: ${duplicateSkillNames.join(', ')}. Tidak ada skill baru untuk ditambahkan.`, 'info');
         setIsSaving(false);
         return;
    }
    ...
    try {
      ...
      console.error("Failed to add hard skills:", err);
  ...
  return (
    <div className="fixed inset-0 bg-opacity-40 backdrop-blur-md flex justify-center items-center z-[100] p-4">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 left-20 w-32 h-32 bg-gradient-to-r from-orange-400/20 to-red-400/20 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute bottom-20 right-20 w-40 h-40 bg-gradient-to-r from-red-400/20 to-orange-400/20 rounded-full blur-3xl animate-pulse delay-1000"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-24 h-24 bg-gradient-to-r from-orange-400/20 to-red-400/20 rounded-full blur-2xl animate-pulse delay-500"></div>
      </div>
      ...
      <div className="relative bg-white/95 backdrop-blur-xl p-6 md:p-8 rounded-3xl shadow-2xl border border-white/20 w-full max-w-lg max-h-[95vh] flex flex-col animate-scale-in">
        {/* Header */}
        <div className="flex justify-between items-center mb-8 pb-6 border-b border-gradient-to-r from-orange-200/50 to-red-200/50">
          ...
          <button 
            onClick={onClose} 
            className="group p-2 rounded-xl text-gray-400 hover:text-orange-600 hover:bg-orange-50 transition-all duration-300 hover:scale-110"
          >
        ...
        <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto pr-2 space-y-6">
          {skillsToAdd.map((skill, index) => (
            <div 
              key={index} 
              className="group p-5 bg-gradient-to-r from-orange-50/80 to-red-50/80 backdrop-blur-sm border border-orange-200/50 rounded-2xl space-y-4 hover:shadow-lg hover:border-orange-300/60 transition-all duration-300 animate-fade-in-up" 
              style={{ animationDelay: `${index * 100}ms` }}
              ref={el => dropdownRefs.current[index] = el}
            >
              {/* Skill Name Input */}
              <div className="relative">
                <label htmlFor={`hardskill-name-${index}`} className="block text-sm font-semibold text-orange-800 mb-2 flex items-center">
                  <Sparkles size={14} className="mr-1.5 text-orange-600" />
                  Nama Hard Skill
                </label>
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 text-sm rounded-xl border border-orange-200/60 bg-white/80 backdrop-blur-sm focus:border-orange-400 focus:ring-2 focus:ring-orange-200/50 transition-all duration-300 placeholder-gray-400"
                  ...
                  {showDropdowns[index] && (
                    <>
                      {isLoadingMasterSkills && (
                        <div className="absolute z-20 w-full bg-white/95 backdrop-blur-sm border border-orange-200/60 rounded-xl mt-2 p-4 text-sm text-gray-500 shadow-xl animate-fade-in">
                          <div className="flex items-center justify-center space-x-2">
                            <div className="w-4 h-4 bg-gradient-to-r from-orange-400 to-red-400 rounded-full animate-pulse"></div>
                      ...
                      {!isLoadingMasterSkills && getFilteredMasterSkills(index).length > 0 && (
                        <ul className="absolute z-20 w-full bg-white/95 backdrop-blur-sm border border-orange-200/60 rounded-xl mt-2 max-h-40 overflow-y-auto shadow-xl animate-fade-in">
                          {getFilteredMasterSkills(index).map(ms => (
                            <li 
                              key={ms.id} 
                              onClick={() => handleSelectMasterSkill(index, ms.name)}
                              className="px-4 py-3 text-sm hover:bg-gradient-to-r hover:from-orange-50 hover:to-red-50 cursor-pointer transition-all duration-200 border-b border-orange-100/50 last:border-b-0 first:rounded-t-xl last:rounded-b-xl"
                            >
                              <div className="flex items-center">
              ...
              {/* Skill Level Select */}
              <div>
                <label htmlFor={`hardskill-level-${index}`} className="block text-sm font-semibold text-orange-800 mb-2 flex items-center">
                  <Star size={14} className="mr-1.5 text-orange-600" />
                  Level Kemahiran
                </label>
                <div className="relative">
                  <select
                    ...
                    className="w-full px-4 py-3 text-sm rounded-xl border border-orange-200/60 bg-white/80 backdrop-blur-sm focus:border-orange-400 focus:ring-2 focus:ring-orange-200/50 appearance-none transition-all duration-300 cursor-pointer"
                    disabled={isSaving}
                  >
          ...
          <button 
            ...
            className="group flex items-center justify-center w-full p-4 bg-gradient-to-r from-orange-50 to-red-50 hover:from-orange-100 hover:to-red-100 border-2 border-dashed border-orange-300/50 hover:border-orange-400/60 rounded-2xl text-orange-600 hover:text-orange-700 transition-all duration-300 hover:scale-105"
            disabled={isSaving}
          >
        ...
        {/* Footer Actions */}
        <div className="pt-6 mt-6 border-t border-gradient-to-r from-orange-200/50 to-red-200/50 flex justify-end space-x-4">
          <button 
            type="button" 
            onClick={onClose} 
          ...
          <button 
            ...
            className="group bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white text-sm font-medium py-3 px-6 rounded-xl transition-all duration-300 flex items-center disabled:opacity-60 hover:scale-105 hover:shadow-lg" 
            disabled={isSaving || isLoadingMasterSkills}
          >
            {isSaving ? (
              <>
